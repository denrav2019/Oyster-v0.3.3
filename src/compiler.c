...
static int add_string(const char* str) {
    ...
    // Поиск в str_pool_buf
    const char* p = str_pool_buf;
    const char* end = str_pool_buf + str_pool_pos;

    while (p < end) {
        if (p[0] == '<' && p[1] == 'l' && p[2] == ':') {
            p += 3;
            char len_str[5] = {p[0], p[1], p[2], p[3], '\0'};
            int len = (int)strtoul(len_str, NULL, 16);
            p += 4;
            if (*p != '>') break;
            p++;
            if (len == (int)strlen(str) && memcmp(p, str, len) == 0) {
                // Нашли дубликат — вычисляем индекс
                int count = 0;
                const char* q = str_pool_buf;
                while (q < p - len) {
                    if (q[0] == '<' && q[1] == 'l' && q[2] == ':') {
                        count++;
                        q += 3;
                        int l = (int)strtoul(q, NULL, 16);
                        q += 4 + 1 + l;
                    } else break;
                }
                // Сохраняем в string_pool
                string_pool[string_pool_count] = my_strdup(str);
                string_pool_count++;
                return count;
            }
            p += len;
    ...
    // Новая строка
    int idx = string_pool_count;
    string_pool[idx] = my_strdup(str);
    string_pool_count++;
    return idx;
}
...
static void ctx_push(CtxType type) {
    ...
    f->type = type; f->entry = 0; f->exit_false = 0; f->exit_true = 0; f->has_else = 0;
}
...
static int parse_atom(const char** p, char* code_buf, int* code_pos, char* str_pool_buf, int* str_pool_pos, int* has_code) {
    ...
    if (**p == '"' || **p == '\'') {
        ...
        // Поиск дубликата в str_pool_buf
        int idx = -1;
        const char* bp = str_pool_buf;
        const char* bend = str_pool_buf + *str_pool_pos;
        int count = 0;
        //fprintf(stderr, "DEBUG string '%s': idx=%d, str_pool_pos=%d\n", str, idx, *str_pool_pos);
        ...
        if (idx < 0) {
            // Новая строка
            add_string(str);  // добавляем в string_pool (для совместимости)
            idx = count;  // индекс = текущее количество строк в str_pool_buf
            *str_pool_pos += snprintf(str_pool_buf + *str_pool_pos, STR_POOL_BUF_SIZE - *str_pool_pos, "<l:%04X>%s", len, str);
            //idx = add_string(str);
            //*str_pool_pos += snprintf(str_pool_buf + *str_pool_pos, STR_POOL_BUF_SIZE - *str_pool_pos, "<l:%04X>%s", len, str);
        }
        // else: дубликат найден, idx уже установлен, str_pool_buf не меняем
...
static int parse_statement(const char** p, char* code_buf, int* code_pos, char* str_pool_buf, int* str_pool_pos, int* has_code, int source_mode) {
    ...
    if (is_keyword(*p, "if", 2)) {
        const char* saved_line = *p; *p += 2; while (**p == ' ') (*p)++;
        if (**p != '(') { fprintf(stderr, "Expected '('\n"); return 0; } (*p)++; while (**p == ' ') (*p)++;
        ...
        *code_pos += snprintf(code_buf + *code_pos, CODE_BUF_SIZE - *code_pos, "<f:00000000>");
        ...
        if (is_keyword(*p, "else", 4)) {
            *p += 4; while (**p == ' ') (*p)++; if (**p != '{') { fprintf(stderr, "Expected '{'\n"); return 0; } (*p)++;
            f->has_else = 1;
            int ejp = *code_pos;
            *code_pos += snprintf(code_buf + *code_pos, CODE_BUF_SIZE - *code_pos, "<j:00000000>");
            ...
            patch_jump(code_buf, ejp, f->exit_true);
        } else {
            f->exit_true = f->exit_false;
            patch_jump(code_buf, jp, f->exit_true);
        }
        ctx_pop();
        if (source_mode) emit_source_comment(code_buf, code_pos, saved_line, source_mode);
        return 1;
    ...
    if (is_keyword(*p, "for", 3)) {
        const char* saved_line = *p; *p += 3; while (**p == ' ') (*p)++;
        int is_in_style = 0;
